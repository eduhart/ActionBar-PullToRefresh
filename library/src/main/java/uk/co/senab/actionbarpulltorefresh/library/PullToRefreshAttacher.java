...
public class PullToRefreshAttacher {
  ...
  protected PullToRefreshAttacher(Activity activity, Options options) {
    ...
    if(Build.VERSION.SDK_INT >= 21 && Build.MODEL.equals("SM-T800")) {
      mStatusBarSize = activity.getResources().getDimensionPixelSize(R.dimen.container_top_padding);
    }
    else{
      mStatusBarSize = 0;
    }
  ...
  protected void addHeaderViewToActivity(View headerView) {
    ...
    wlp.x = 0;
    wlp.y = mRect.top + mStatusBarSize;
  ...
  protected void updateHeaderViewPosition(View headerView) {
    ...
    if (wlp != null && wlp.y != (mRect.top + mStatusBarSize)) {
      wlp.y = mRect.top + mStatusBarSize;
